...
- 面向具备通用技术背景、第一次接触实现的读者；先说明谁在何时做什么、产生什么结果，再绑定代码符号与证据。
- 把输入视为事实材料，按当前读者重组措辞；标题与段落以“参与者 + 运行时动作 + 结果”为常用骨架。
- 术语首次出现时说明角色，后文沿用同一名称；范围变化时重新点明主体。图中节点保持精炼，正文用完整句子连接因果。

## 输出构造

1. **一句话结论**：先回答用户真正关心的问题。
2. **主关系**：把核心问题写成“起点 → 关键跃迁 → 终态”，选择一个原语完整承载。优先让空间、连接、对齐、形态或状态编码直接呈现关系；文本为读者串起主体、条件、因果和边界。
...
6. **可寻址信息**：信息具有已知、精确且可访问的目标地址，且跳转有助于验证或继续探索时，将承载其语义的最小完整文本原位链接；不猜测地址，不建立脱离上下文的链接墙。
7. **实现附件**：用户需要完整实现时，将其作为独立附件层交付。
...
- 关系比清单更醒目，重点比装饰更醒目。
- HTML 具有明确的视觉主次与充分留白；低密度指降低同一视区的认知负荷，而不是减少必要信息。
- 名称、状态与结论能够回到真实证据。
