...
def is_approved_font_stylesheet(value: str) -> bool:
    try:
    ...
    if host == "fonts.googleapis.com" and parsed.path in {"/css2", "", "/"}:
        return True
    if host in {"cdn.jsdelivr.net", "fonts.gstatic.com"}:
        return True
    return False
...
def reference_error(tag: str, rel: str, value: str) -> str | None:
    ...
    if not remote:
        ...
        return None
    if tag == "link":
        rel_tokens = rel.casefold().split()
        if "stylesheet" in rel_tokens or "preconnect" in rel_tokens:
            if is_approved_font_stylesheet(stripped):
                return None
            return f"remote stylesheet/font is not an approved URL: {stripped[:80]}"
