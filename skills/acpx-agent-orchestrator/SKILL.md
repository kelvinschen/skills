...
description:  当明确需要使用 acpx  来编排专用 coding agents 时使用此技能。它涵盖 acpx 设置、one-shot、named session、flow 编排，以及在已注册 acpx agents（例如：`aiden`、`trae`、 `claude`、`codex`、`pi` 等）之间路由工作。
---
...
Flow templates 包含默认 profiles。`simple-feature` defaults 为 `PLAN_AGENT=aiden`、`IMPLEMENT_AGENT=trae` 和 `VALIDATE_AGENT=aiden`；`complex-feature-refactor` 额外包含 `PLAN_REVIEW_AGENT=trae`；`quick-bugfix` 支持 `IMPLEMENT_AGENT` 和 `TEST_AGENT`。环境变量会覆盖 input role fields。如果 caller 确认了 handoff location，在 flow input 中传入 `handoffDir`；否则节点使用 `<repo>/tmp/flow_handoffs/<runId>/<node>.md`，shared memory index 位于 `<repo>/tmp/flow_handoffs/<runId>/flow-memory.md`。
