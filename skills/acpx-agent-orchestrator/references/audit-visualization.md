...
对已结束的 flow/session audit reports，优先使用 `scripts/acpx-inspector report ...`；需要兼容旧报告或单独使用 visualizer 时，使用 `scripts/acpx-visualize`。`scripts/acpx-visualize` 有意不作为 state tracker、helper index、poller 或 live agent work 的 wrapper。
...
scripts/acpx-inspector report flow --run-id <runId> --output /tmp/acpx-audit.html
scripts/acpx-inspector report session --agent <agent> --cwd /repo --name impl --output /tmp/acpx-session-audit.html
```
...
此工具用于 post-run audit，不用于 live progress tracking。Live tracking 应使用 `scripts/acpx-inspector follow`；`projections/live.json` 和 `sessions read --tail` 只作为 fallback/debug。
